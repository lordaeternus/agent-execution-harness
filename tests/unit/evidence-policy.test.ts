...
import { verifyClaim } from "../../src/core/claims.js";
...
describe("evidence policy coverage", () => {
  it("keeps run partial when explicit file_scope evidence is missing", () => {
    ...
    expect(verifyClaim(state, { claim_id: "accept", kind: "acceptance_criteria_met", value: "coverage-task", evidence_id: "ev" }, config).verified).toBe(false);
    state = processHarnessAction({ plan, previousState: state, runId: "coverage-run", mode: "standard", config, action: { schema_version: ACTION_SCHEMA_VERSION, type: "verify_claims", claims: [ { claim_id: "gate", kind: "gate_passed", value: "node --version", evidence_id: "ev" }, { claim_id: "task", kind: "task_reconciled", value: "coverage-task", evidence_id: "ev" }, { claim_id: "rollback", kind: "rollback_defined", value: "Revert coverage fixture.", evidence_id: "ev" } ] } }).state;
