...
export function verifyClaim(state: AgentHarnessRunState, claim: HarnessClaim, config: AgentHarnessConfig): VerifiedClaim {
  ...
  if (verified && claim.kind === "acceptance_criteria_met") {
    verified = state.tasks.some((task) => task.task_id === claim.value && task.status === "completed" && task.evidence_ids.includes(claim.evidence_id)) && isTaskEvidenceComplete(state, claim.value);
  }
  if (verified && claim.kind === "contract_preserved") verified = isTaskEvidenceComplete(state, claim.value) && /contract|governance|typecheck|test/i.test(evidence?.scope_covered ?? "");
